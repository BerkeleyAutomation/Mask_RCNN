...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, gt_targets, config):
    ...
    gt_targets: None if target branch not enabled, otherwise
                [MAX_GT_INSTANCES] int target y/n (1/0)
    ...
    (Only when target branch is enabled)
    targets: [TRAIN_ROIS_PER_IMAGE]. Integer target status (1/0). Zero padded.
...
class DetectionTargetLayer(KE.Layer):
    ...
    Output signature will vary depending on which optional branches are enabled-
    see compute_output_shape() and compute_mask()
    ...
    gt_targets: None if target branch not enabled, otherwise
                [batch, MAX_GT_INSTANCES] Target statuses (0/1)
    ...
    target_mask: [batch, TRAIN_ROIS_PER_IMAGE, height, width]
                 ...
                 network output size.
    (Only when target branch is enabled)
    target_is_target: [batch, TRAIN_ROIS_PER_IMAGE]. Integer target y/n (1/0)
    ...
    def call(self, inputs):
        ...
        outputs = utils.batch_slice(
            [proposals, gt_class_ids, gt_boxes, gt_masks, gt_targets],
            lambda w, x, y, z, k: detection_targets_graph(
                w, x, y, z, k, self.config),
...
def refine_detections_graph(rois, probs, deltas, target_probs, window, config):
    ...
    Inputs:
        ...
        deltas: [N, num_classes, (dy, dx, log(dh), log(dw))]. Class-specific
                bounding box deltas.
        target_probs: None if target branch not enabled, otherwise
                      [N, 2]. Target probabilities
...
class DetectionLayer(KE.Layer):
    ...
    If target branch enabled, then return
    ...
    def call(self, inputs):
        ...
        detections_batch = utils.batch_slice(
            [rois, mrcnn_class, mrcnn_bbox, target_probs, window],
            lambda x, y, w, p, z: refine_detections_graph(x, y, w, p, z, self.config),
            self.config.IMAGES_PER_GPU)
    ...
    def compute_output_shape(self, input_shape):
        return (None, self.config.DETECTION_MAX_INSTANCES, self.detection_dim)
...
def fpn_target_graph(rois, feature_maps, target_feature_maps, image_meta,
    ...
    # Classifier head
    target_logits = KL.TimeDistributed(KL.Dense(2),
                                            name='siamese_logits')(shared)
    target_probs = KL.TimeDistributed(KL.Activation("softmax"),
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    # [batch, num_boxes, pool_size, pool_size, channels].
...
def target_branch_loss_graph(gt_targets, pred_logits, pred_target_prob, siamese_output, features, target_feature, positive_weight):
    """Loss for target branch of Mask RCNN. Implemented based off of
    mrcnn_class_loss_graph.

    positive_weight controls the relative weight of the losses of the target vs.
    other detections. A value of 5, for example, means that the loss value
    corresponding to the target RoI will be multiplied by 5.

    gt_targets: [batch, num_rois]. For each roi, 0 if not target,
                1 if so. Uses 0-padding.
    ...
    # Temporary, should be the same as in fpn_target_graph
    # target_prob = KL.TimeDistributed(KL.Activation('softmax'), name='target_prob_loss')(pred_logits)

    # loss = tf.abs(pred_target_prob[:,:,1] - gt_targets, name='abs_loss')
...
def load_target(target_dataset, config, target_id):
    ...
    target_vector = np.zeros_like(class_ids)

    # support no target at all
    if target_index >= 0:
...
def load_image_gt(dataset, config, image_id, use_mini_mask=False):
    ...
    """
    # Load image and mask
    example = dataset.load_example(image_id)
    image = example['pile_image']
    mask = example['pile_mask']
    ...
    source_class_ids = dataset.source_class_ids[dataset.example_info[image_id]["source"]]
...
# Temporary: until dataset is fixed
...
def load_inputs_gt(dataset, config, image_id):
    ...
    - extra_inputs: a list of extra inputs from the Dataset, empty if none.
    """
...
def data_generator(dataset, config, shuffle=True,
    ...
    dataset: The TargetDataset object to pick data from
    config: The model config object
    ...
    *FIXED AT 1 PENDING FURTHER REFACTOR*
    ...
    image_index = -1
    image_ids = np.copy(dataset.example_indices)
    error_count = 0
    ...
    while True:
        try:
            ...
            # RPN Targets
            rpn_match, rpn_bbox = build_rpn_targets(pile_image.shape, anchors,
                                                    gt_class_ids, gt_boxes, config)

            # Mask R-CNN Targets
            if random_rois:
                rpn_rois = generate_random_rois(
                    pile_image.shape, random_rois, gt_class_ids, gt_boxes)
            ...
            if b == 0:
                ...
                batch_images = np.zeros(
                    (batch_size,) + pile_image.shape, dtype=np.float32)
            ...
            batch_images[b] = mold_image(pile_image.astype(np.float32), config)
            ...
            logging.exception("DataException on image {}".format(dataset.example_info[image_id]))
        except:
            # Log it and skip the image
            logging.exception("Error processing image {}".format(
                dataset.example_info[image_id]))
            error_count += 1
            if error_count > 5:
                raise
...
class MaskRCNN():
        ...
        if mode == "training":
            ...
            if self.target_branch:
                ...
                input_target_gt_index = KL.Input(
                    shape=[None], name="input_target_gt_index", dtype=tf.int32
                )
            else:
                input_target_gt_index = None
    ...
    def run_graph(self, images, targets, target_bbs, outputs, image_metas=None, target_metas=None, gt_inputs=None):
        ...
        if image_metas is None:
            ...
            molded_targets, target_metas, _ = self.mold_inputs(targets)
        else:
            molded_images = images
            molded_targets = targets
        image_shape = molded_images[0].shape
        # Anchors
        anchors = self.get_anchors(image_shape)
        # Duplicate across the batch dimension because Keras requires it
        # TODO: can this be optimized to avoid duplicating the anchors?
        anchors = np.broadcast_to(anchors, (self.config.BATCH_SIZE,) + anchors.shape)
        model_in = [molded_targets, target_metas, target_bbs, molded_images, image_metas, anchors]
    ...
    def run_graph_train(self, base_inputs, extra_inputs,
                        desired_outputs=None):
        """Frankenstein-ed together from model.train, model.run_graph, and
        data_generator"""
        assert self.mode == "training", "Create model in training mode."
        ...
        # Train
        log("Evaluating the following graph outputs in training mode:\n {}".format(
            [tensor.name for (string, tensor) in desired_outputs]))
        log("Checkpoint Path: {}".format(self.checkpoint_path))
        ...
        outputs = OrderedDict(desired_outputs)
        for o in outputs.values():
            assert o is not None
        ...
        # model_in, model_labels = next(train_generator) # input examples & gt labels

        model_in = [batch_target_images, batch_target_bb, batch_target_image_meta,
                  batch_target_vector] + [batch_images, batch_image_meta, batch_rpn_match,
                                          batch_rpn_bbox, batch_gt_class_ids,
        ...
        ####
        #### TWO SEPARATE RUNS means that RoIs could be shuffled differently.
        #### Visualization mismatches happen here.
        ####
        # attempting to predict
        log("Performing full prediction")
        pred_outputs = self.keras_model.predict(model_in, verbose=1)

        pred_outputs_keys = ['rpn_class_logits', 'rpn_class', 'rpn_bbox',
                       'mrcnn_class_logits', 'mrcnn_class', 'mrcnn_bbox', 'mrcnn_mask',
                             'rpn_rois', 'output_rois', 'pred_target_logits', 'pred_target_prob',
                       'rpn_class_loss', 'rpn_bbox_loss', 'class_loss,' 'bbox_loss', 'mask_loss', 'target_loss']

        outputs_full = OrderedDict([(k, v) for k, v in zip(pred_outputs_keys, pred_outputs)])


        # run full detection
        return outputs_np, model_in, model_labels, outputs_full
